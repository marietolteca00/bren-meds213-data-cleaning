...
```
- To make this into a database, it seems structured, But there is no primary key. 
- To make a database the primary key would be a combination of; Site, Plot, Location, Date
...
#   # If there is '.' replace with NA in the Snow_cover column
#   mutate(Snow_cover = ifelse(Snowcover == ".", NA, Snow_cover))
snowsurvey_fixed <- snowsurvey_csv %>%
...
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = ifelse(Snow_cover == ".", NA, Snow_cover))
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = ifelse(Snow_cover == "n/a", NA, Snow_cover))
```
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = ifelse(Snow_cover == "unk", NA, Snow_cover))  
```
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = ifelse(Snow_cover == "<1", "0", Snow_cover))  
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = as.numeric(Snow_cover))  
```
...
- There are two values over 100. One is -200 but total cover is 100, and the other is a 470 value, We will just make those NA for now
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snowcover = ifelse(Snow_cover == ">100", NA, Snow_cover))  
```
...
species_presence <- species_long %>%
  mutate(Presence = ifelse(Presence == 0,
                           #if its not true we want 1
                           0, 1))
